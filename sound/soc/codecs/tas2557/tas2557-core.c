...
//Reading cal data from mfd during init
#include "fih/fih_mem.h"

static unsigned int my_proc_addr = 0xAFA81000;//FIH_MEM_SAMP_ADDR
static unsigned int my_proc_size = 0x00001000;//FIH_MEM_SAMP_SIZE
//Reading cal data from mfd during init
...
static void failsafe_reload_firmware(struct tas2557_priv *pTAS2557)
{
	dev_err(pTAS2557->dev, "%s\n", __func__);
	failsafe(pTAS2557);
	request_firmware_nowait(THIS_MODULE, 1, tas2557_fw_bin,
...
int tas2557_enable(struct tas2557_priv *pTAS2557, bool bEnable)
{
	...
	if (nResult < 0) {
		if (pTAS2557->mnErrCode & (ERROR_DEVA_I2C_COMM | ERROR_PRAM_CRCCHK | ERROR_YRAM_CRCCHK))
			failsafe_reload_firmware(pTAS2557);
	}
...
// -----------------------------------------------------------------------------
// Load data from FIH mfd
// -----------------------------------------------------------------------------
// Description:
//      Load calibration data from FIH mfd (manufacture data)
// -----------------------------------------------------------------------------
...
static int tas2557_load_calibration(struct tas2557_priv *pTAS2557,	char *pFileName)
{
...
#else //read calibration data from mfd
	...
	if (buf == NULL) {
		dev_err(pTAS2557->dev, "%s: ioremap fail\n", __func__);
		goto end;
	}

	dev_err(pTAS2557->dev, "TBC: sizeof(reginfo)=%ld, start memcpy from %p\n", sizeof(reginfo), (void*)buf);
	memcpy(&reginfo, (void *)buf, sizeof(reginfo));
	dev_err(pTAS2557->dev, "Magic=0x%x,Cal_Info=0x%x,Len=%d\r\n", reginfo.Magic, reginfo.Cal_Info, reginfo.len);

	if (reginfo.len > 0) {
		nSize = reginfo.len;
		dev_err(pTAS2557->dev, "TBC: nSize=%d, start memcpy from %p\n", nSize, (void*)(buf+sizeof(reginfo)));
		memcpy(pfwBuffer,  buf+sizeof(reginfo), nSize);
#if 0 //dump calibration data
		{
			int i=0;
			for (i = 0; i < ((nSize/16) + 1); i++) {
				dev_dbg(pTAS2557->dev, "0x%08x: %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x\r\n", i*16,
...
#endif
		dev_info(pTAS2557->dev, "Calibrarion data read successfully (nSize:%d)\r\n", nSize);
	}
	...
	if (reginfo.Magic!=SNS_REG_MAGIC_V01) {
		dev_err(pTAS2557->dev, "Verify Calibrarion data fail: wrong Magic:0x%x \r\n", reginfo.Magic);
	goto end;
	}

	if (reginfo_tail.Magic!=SNS_REG_MAGIC_TAIL) {
		dev_err(pTAS2557->dev, "Verify Calibrarion data fail: wrong tail Magic:0x%x \r\n", reginfo_tail.Magic);
		goto end;
    }
	...
	if (nResult)
		...
		dev_err(pTAS2557->dev, "TAS2557 calibration: %d calibrations\n",
			pTAS2557->mpCalFirmware->mnCalibrations);
...
void tas2557_fw_ready(const struct firmware *pFW, void *pContext)
{
	...
	//force to disable tas2557 after init
	nResult = tas2557_enable(pTAS2557, 0);
...
int tas2557_parse_dt(struct device *dev, struct tas2557_priv *pTAS2557)
{
	...
	if (!rc) {
		/* Project related firmware name */
		if (strcmp(device_model, "PNX") == 0) {
			tas2557_fw_bin = "PNX_TAS2557.bin";
		} else {
