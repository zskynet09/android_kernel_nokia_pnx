...
char *Firmware_Tianma = "Himax_firmware_Tianma.bin";
char *Firmware_CTC = "Himax_firmware_CTC.bin";
char *Firmware_default = "Himax_firmware.bin";

char i_CTPM_firmware_name[32] = "";
...
extern unsigned int touch_double_tap_read(void);
extern int touch_double_tap_write(unsigned int enable);
extern struct fih_touch_cb touch_cb;
static unsigned int double_tap_enable;
//FIH touch -- end

//Add dsi notifier callback +[
static int dsi_panel_notifier_callback(struct notifier_block *self,
				 unsigned long event, void *data)
{
	int blank;
	struct msm_drm_notifier *evdata = data;
	struct himax_ts_data *ts = container_of(self, struct himax_ts_data, dsi_panel_notif);
	...
	if (ts && (event == MSM_DRM_EARLY_EVENT_BLANK)) {
		...
		if (blank == MSM_DRM_BLANK_UNBLANK) {
			I("%s: receives EARLY_BLANK:UNBLANK\n", __func__);
		} else if (blank == MSM_DRM_BLANK_POWERDOWN) {
			I("%s: receives EARLY_BLANK:POWERDOWN\n", __func__);
			himax_chip_common_suspend(ts);
		} else {
			I("%s: receives wrong data EARLY_BLANK:%d\n",
				__func__, blank);
		}
	}
	...
	if (ts && event == MSM_DRM_EVENT_BLANK) {
		...
		if (blank == MSM_DRM_BLANK_POWERDOWN) {
			I("%s: receives BLANK:POWERDOWN\n", __func__);
		} else if (blank == MSM_DRM_BLANK_UNBLANK) {
			I("%s: receives BLANK:UNBLANK\n", __func__);
			himax_chip_common_resume(ts);
		} else {
			I("%s: receives wrong data BLANK:%d\n",
				__func__, blank);
		}
	}
	return 0;
...
static int i_get_FW(void)
{
	...
	memset(i_CTPM_firmware_name, '\0', strlen(i_CTPM_firmware_name));

	switch(get_Display_ID())
	{
		case DISPLAY_PANEL_ID_HX83112A_TIANMA_FHD_VIDEO:
			scnprintf(i_CTPM_firmware_name, strlen(Firmware_Tianma) + 1, "%s", Firmware_Tianma);
			break;
		case DISPLAY_PANEL_ID_HX83112A_CTC_FHD_VIDEO:
			scnprintf(i_CTPM_firmware_name, strlen(Firmware_CTC) + 1, "%s", Firmware_CTC);
			break;
		case DISPLAY_PANEL_ID_NOT_DEFINED:
		case DISPLAY_PANEL_ID_FT8719A_CTC_FHD_VIDEO:
		default:
			scnprintf(i_CTPM_firmware_name, strlen("Invalid_Panel"), "%s", "Invalid_Panel");
			break;
	}
...
int himax_checksum_cal(struct himax_ts_data *ts,uint8_t *buf,int ts_status)
{
    ...
    {
#ifdef HX_ESD_RECOVERY
	if (hx_esd_event == length && g_esd_event_cnt == 0) {
		g_esd_event_cnt++;
...
#ifdef CONFIG_FB
//From sdm670/710, should register drm_notifier instead of fb_notifier
#if 0
...
//Add dsi notifier callback +[
static void himax_drm_register(struct work_struct *work)
{
    int ret = 0;
    struct himax_ts_data *ts = container_of(work, struct himax_ts_data,
                                            work_att.work);
...
int himax_chip_common_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    I("%s:180629 modification03!\n", __func__);
    I("%s:Panel ID = %d\n", __func__, get_Display_ID());

    //Check touch IC, if IC is not match, return err
    if (get_Display_ID() != DISPLAY_PANEL_ID_HX83112A_TIANMA_FHD_VIDEO && get_Display_ID() != DISPLAY_PANEL_ID_HX83112A_CTC_FHD_VIDEO)
    {
        I("%s: Touch IC is not match!\n", __func__);
        return err;
    }
...
FW_force_upgrade:
    ...
    INIT_DELAYED_WORK(&ts->work_att, himax_drm_register);
...
int himax_chip_common_suspend(struct himax_ts_data *ts)
{
    ...
    if(ts->SMWP_enable)
    {
        ...
        I("Enter gesture mode enable irq\n");
        return 0;
    }
#endif
    ...
    double_tap_enable = ts->SMWP_enable; //Make sure double tap state is correct when chip suspend

    if (double_tap_enable == 0)
    {
        I("FIH double off TP susepnd reset down \n");
...
int himax_chip_common_resume(struct himax_ts_data *ts)
{
    ...
    double_tap_enable = ts->SMWP_enable; //Make sure double tap state is correct when chip resume

    if (double_tap_enable == 0)
    {
         I("FIH double off TP resume reset high \n");
