                                   ...
                                   size_t len, loff_t *pos)
{
    ...
    if(!HX_PROC_SEND_FLAG)
    {
        himax_sense_off(private_ts->client, true);
        msleep(20);
...
char *g_hx_crtra_file = "hx_criteria.csv";
								...
								size_t len, loff_t *pos)
{
	...
	/*if (buf[0] == '1') {
		g_hx_crtra_file = "hx_criteria.csv";
		E("%s: file_anme=%s!\n", __func__, g_hx_crtra_file);
	} else if (buf[0] == '2') {
		g_hx_crtra_file = "hx_criteria2.csv";
		E("%s: file_anme=%s!\n", __func__, g_hx_crtra_file);
	} else {
		E("%s: Fail command!\n", __func__);
		return -EINVAL;
                                    ...
                                    size_t len, loff_t *pos)
{
    ...
    if(!HX_PROC_SEND_FLAG)
    {
        ...
        result = himax_chip_self_test(private_ts->client);
...
void touch_tpfwver_read(char *fw_ver)
{
	...
	if(g_f_hx_fw_update == 1) {
		...
		sprintf(fw_ver, "Himax-0x%2.2X_0x%2.2X \n",ic_data->vendor_fw_ver, ic_data->vendor_touch_cfg_ver);
	}
	I("%s: END!\n", __func__);
}
